 ...
 */ 
global $super_options, $rad_flag; 

/**
 * RAD Builder Conditional Check
 * Only works if user is logged in and has admin permissions.
 */

if( isset($_GET['rad']) && $_GET['rad']=="on" &&  current_user_can('delete_pages') )
{
  get_template_part("templates/rad/init");
  die(); // Don't worry, it's for RAD mode only.
}
...
<body <?php body_class(); ?> itemscope itemtype="http://schema.org/WebPage" > <!-- Start of body  -->
...
<div class="super-wrapper   clearfix">
  <div class="inner-super-wrapper <?php if( isset($super_options[SN.'_boxed_layout']) && $super_options[SN.'_boxed_layout']=="true") echo 'ioa-boxed-layout' ?>" >  
