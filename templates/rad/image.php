...
<div <?php echo join(" ",$rad_attrs) ?>>
	<div class="<?php echo $joint ?>-inner-wrap" style='<?php if(isset($w['col_alignment'])) echo "text-align:".$w['col_alignment'] ?>'>
		...
		<?php echo do_shortcode("[image effect_delay='".$w['delay']."' lightbox='".$w['lightbox']."' src='".$w['image']."' effect='".$w['visibility']."' hoverbg='".$w['hoverbg']."' hoverc='".$w['hoverc']."' resize='".$w['resizing']."'  link='".$w['link']."' frame='".$w['frame']."' width='".$w['width']."' height='".$w['height']."' /]"); ?>

		<?php if(isset($w['text_caption'])) : ?><h4 itemprop="description" class=" text_caption <?php if($hidden_data['text_caption']) echo $hidden_data['text_caption']; ?> custom-font1"><?php echo $helper->format($w['text_caption'],true,false,false); ?></h4><?php endif; ?>
