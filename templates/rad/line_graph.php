...
<div <?php echo join(" ",$rad_attrs) ?>>
	<div class="<?php echo $joint ?>-inner-wrap" style='<?php if(isset($w['col_alignment'])) echo "text-align:".$w['col_alignment'] ?>'>
		...
		$code = "[line_chart xlabels='".$w['labels']."' width='".$w['width']."' height='".$w['height']."']";
			foreach ($values as $key => $value) {
				$code .= "[line pointcolor='".$value['line_pointcolor']."'  pointstrokecolor='".$value['line_pointstrokecolor']."'  background='".$value['line_color']."' data='".$value['bar_value']."' strokecolor='".$value['line_stroke']."'/]";
			}
		$code .= "[/line_chart]";
