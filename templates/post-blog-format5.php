...
global $helper,$meta_data,$paged,$super_options;
...
{
   $meta_data['height'] = 350; $meta_data['width'] = 750; 
   if($meta_data['layout']=="full") 
   {
     $meta_data['height'] = 450; $meta_data['width'] = 1060; 
   }
}
else
{
...
$format_type = get_post_format();
  ...
  if ((function_exists('has_post_thumbnail')) && (has_post_thumbnail()))  $meta_data['hasFeaturedImage'] = true;  
   ...
   switch ($format_type) {
    ...
    default: ?>
        ...
        <li itemscope itemtype="http://schema.org/Article"  data-dbg='<?php echo $dbg; ?>' data-dc='<?php echo $dc; ?>'  id="post-<?php the_ID(); ?>"  class="iso-item <?php if($meta_data['j']==0 && $paged==0 ) echo "featured" ?> clearfix <?php echo join(' ',get_post_class()); ?> <?php  if( ! ($meta_data['j']==0 && $paged==0 ))  $meta_data['i']++; if($meta_data['i']==$meta_data['item_per_rows']) {  $meta_data['i']=0; } ?>">
 ...
 <?php break;
  ...
  if($meta_data['j']==0 && $paged==0 ) echo "<li class='spacer'></li></ul><ul itemscope itemtype=\"http://schema.org/Article\"  class='isotope clearfix blog_posts filterable'>";
  $meta_data['j']++;
 ?>  
